...
class MMiDP3Workspace(BaseWorkspace):
    ...
    def run(self):
        ...
        writer.add_text("config/output_dir", str(self.output_dir))
            ...
            for local_epoch_idx in tqdm.tqdm(range(cfg.training.num_epochs), desc=f"Training"):
                ...
                for batch_idx, batch in enumerate(train_dataloader):
                    ...
                    if not is_last_batch:
                        ...
                        writer.add_scalar('Loss/train', step_log['train_loss'], self.global_step)
                        writer.add_scalar('Learning Rate', step_log['lr'], self.global_step)
                        writer.add_scalar('Epoch', step_log['epoch'], self.global_step)
                        for key, value in loss_dict.items():
                            writer.add_scalar(f'Loss/{key}', value, self.global_step)
                ...
                # log of last step is combined with validation and rollout
                # wandb_run.log(step_log, step=self.global_step)
                writer.add_scalar('Loss/train_loss', step_log['train_loss'], self.global_step)
                if (self.epoch % cfg.training.val_every) == 0:
                    writer.add_scalar('Loss/train_action_mse_error', step_log['train_action_mse_error'], self.global_step)
                    writer.add_scalar('Score/test_mean_score', step_log['test_mean_score'], self.global_step)
                writer.add_scalar('Learning Rate', step_log['lr'], self.global_step)
                writer.add_scalar('Epoch', step_log['epoch'], self.global_step)
