...
class MarsmindMetaActEpisodeDatasetMM(BaseDataset):
    def __init__(self,
            data_path, 
            pad_before=0,
            pad_after=0,
        ...
        super().__init__()
        cprint(f'Loading MarsmindEpisodeDatasetMM from {data_path}/{task_name}', 'green')
        ...
        for dir_task in os.listdir(os.path.join(data_path)):
            for dir_episode in os.listdir(os.path.join(data_path, dir_task)):
                for dir_metaaction in sorted(os.listdir(os.path.join(data_path, dir_task, dir_episode))):
                    ...
                    #     continue
                    # dir_metaaction = dir_metaaction.lower()
                    # if 'move' in dir_metaaction and (not ('hill' in dir_metaaction or 'forward' in dir_metaaction or 'boundary' in dir_metaaction or 'container' in dir_metaaction)):
                    #     print(dir_metaaction)
                    #     raise ValueError('meta act name error!')
                    # elif 'turn' in dir_metaaction and (not ('left' in dir_metaaction or 'right' in dir_metaaction)) and\
                    #       (not ('90' in dir_metaaction or '360' in dir_metaaction)):
                    #     print(dir_metaaction)
                    #     raise ValueError('meta act name error!')
                    # elif 'cross' in dir_metaaction and (not ('hill' in dir_metaaction or 'rock' in dir_metaaction or 'wood' in dir_metaaction)):
                    #     print(dir_metaaction)
                    #     raise ValueError('meta act name error!')
                    # elif 'grasp' in dir_metaaction and (not ('wood' in dir_metaaction or 'branch' in dir_metaaction or 'bottle' in dir_metaaction or 'rock' in dir_metaaction \
                    #                                          or 'stone' in dir_metaaction)):
                    #     print(dir_metaaction)
                    #     raise ValueError('meta act name error!')
                    # elif 'place' in dir_metaaction and (not ('wood' in dir_metaaction or 'branch' in dir_metaaction or 'bottle' in dir_metaaction or 'rock' in dir_metaaction \
                    #                                          or 'stone' in dir_metaaction)):
                    #     print(dir_metaaction)
                    #     raise ValueError('meta act name error!')
    ...
    def get_normalizer(self, mode='limits', **kwargs):
        ...
        if self.use_img:
            ...
            normalizer['image_wrist'] = SingleFieldLinearNormalizer.create_identity()
        if self.use_depth:
    ...
    def _sample_to_data(self, sample):
        ...
        if self.use_img:
            ...
            image_wrist = sample['img_wrist'][:,].astype(np.float32)
        if self.use_depth:
        ...
        if self.use_img:
            ...
            data['obs']['image_wrist'] = image_wrist
        if self.use_depth:
...
if __name__ == '__main__':
    dataset = MarsmindMetaActEpisodeDatasetMM(
